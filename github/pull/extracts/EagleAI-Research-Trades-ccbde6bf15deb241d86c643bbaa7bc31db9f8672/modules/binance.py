...
@binance_bp.route("/p/v1/trades/binance/new/", methods=["POST"], strict_slashes=False)
def place_order():
...
@binance_bp.route("/p/v1/trades/binance/<string:symbol>/<int:order_id>", methods=["DELETE"], strict_slashes=False)
def cancel_open_order(symbol, order_id):
    try:
        ...
        user_id = decoded.get("user_id")
        response = binance_service.cancel_open_order(order_id, user_id)
...
@binance_bp.route("/p/v1/trades/binance/open/<string:symbol>", methods=["GET"], strict_slashes=False)
...
@binance_bp.route("/p/v1/trades/binance/account-balance", methods=["GET"], strict_slashes=False)
def get_account_summary():
    try:
       ...
       return (jsonify(
            {
                "status": False,
                "message": e.__dict__.get("error_message") if ERROR_IS_DICT else e
            }
        ), 400)

@binance_bp.route("/p/v1/trades/binance/set-initial-margin", methods=["POST"], strict_slashes=False)
def set_leverage():
    try:
        raw_json = request.json
        if not raw_json:
        ...
        user_id = decoded.get("user_id")
        response = binance_service.set_leverage(body, symbol, user_id)
...
@binance_bp.route("/p/v1/trades/binance/symbols", methods=["GET"], strict_slashes=False)
...
@binance_bp.route("/p/v1/trades/binance/<string:symbol>", methods=["POST"], strict_slashes=False)
...
@binance_bp.route("/p/v1/trades/binance/<string:symbol>", methods=["GET"], strict_slashes=False)
def get_past_orders(symbol):
    try:
        ...
        if response.get("error") != None: raise response.get("error") 
...
@binance_bp.route("/p/v1/trades/binance/fills/<string:symbol>", methods=["GET"], strict_slashes=False)
...
@binance_bp.route("/p/v1/trades/binance/historical-pnl", methods=["GET"], strict_slashes=False)
...
@binance_bp.route("/p/v1/trades/binance/account", methods=["GET"], strict_slashes=False)
